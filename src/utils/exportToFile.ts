...
function addPerformanceRoomsToWb(wb: WorkBook, performanceRooms: PerformanceRoom[]) {
  performanceRooms.forEach((room, i) => {
    const fileModels = room.performances.map((x) => ({
...
function addAuralTestsToWb(wb: WorkBook, auralTests: AuralTest[], auralRoomCount: number) {
  ...
  rowsByRoom.forEach((room, i) => {
    ...
    utils.book_append_sheet(wb, ws, `A ${i + 1}`);
  });
}
...
function exportToFile(
  ...
  addPerformanceRoomsToWb(wb, performanceRooms);
  addAuralTestsToWb(wb, auralTests, auralRoomCount);
